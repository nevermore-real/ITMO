...
public class Updater {
    ...
    public void update(Product product, BufferedReader reader) throws ExecuteException{

        Printer.printUpdate(1);
            ...
            try {
                ...
                if (splitOne.length == 1) { // [coordinates]
                    ...
                    if (Validator.validType(type)) {
                        if (updHardField(type, product, reader)) {
                            Printer.printUpdate(2);
                        }else Printer.printUpdate(3);
                    }
                } else if (splitOne.length == 2){ // [x, 1]

                    if (Validator.validEasyUpdate(splitOne)) {
                        updEasyField(product, splitOne);
                        Printer.printUpdate(4);
                    }
                }
            }catch (IOException e){
    ...
    public void updEasyField(Product product, String[] args) throws ExecuteException {
        String field = args[0];
    ...
    public boolean updHardField(String type, Product product, BufferedReader reader) throws ExecuteException{
